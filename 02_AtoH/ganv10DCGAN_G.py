...
def make_discriminator_model(num_class=2):
    ...
    model.add(layers.Conv3D(1, (4, 4, 4), strides=(2, 2, 2), padding='same'))
    model.add(layers.LeakyReLU(alpha=0.2))
    #model.add(layers.BatchNormalization())
...
def train(path, run_name, dataset, epochs, noise_dim, generator, discriminator, generator_optimizer, discriminator_optimizer, seed, checkpoint, checkpoint_prefix, cross_entropy, acc_real, acc_fake):
  ...
  for epoch in trange(epochs, desc='Epoch'):
    ...
    set_learning_rate(current_learning_rate, generator_optimizer, discriminator_optimizer)
...
def train_gan(path, run_name, num_vox):
    ...
    train_data = ldsv.load_data_for_gan(path, max_files_load=1)
    ...
    train(path, run_name, train_dataset, EPOCHS, noise_dim, generator, discriminator, generator_optimizer, discriminator_optimizer, seed, checkpoint, checkpoint_prefix, cross_entropy, acc_real, acc_fake)
