...
#wgan v K is a gan using Wasserstein loss, logits, LeakyReLU, learning rate decay, label smoothing, and batch normalization
...
def make_generator_model(num_class=2):
    ...
    model.add(layers.Conv3DTranspose(24, (8, 8, 8), strides=(2, 2, 2), padding='same', use_bias=False, kernel_initializer=INIT))
    model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU())
    assert model.output_shape == (None, 40, 40, 20, 24)
    
    model.add(layers.Conv3DTranspose(12, (8, 8, 8), strides=(2, 2, 2), padding='same', use_bias=False, kernel_initializer=INIT))
    model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU())
    assert model.output_shape == (None, 80, 80, 40, 12)

    model.add(layers.Conv3DTranspose(num_class, (8, 8, 8), strides=(2, 2, 2), padding='same', use_bias=False, activation='sigmoid'))
...
def make_discriminator_model(num_class=2):
    model = tf.keras.Sequential()
    model.add(layers.Conv3D(12, (8, 8, 8), strides=(2, 2, 2), padding='same', kernel_initializer=INIT,
                                     input_shape=[160, 160, 80, num_class]))
    model.add(layers.LeakyReLU(alpha=0.2))
    model.add(layers.BatchNormalization())
    assert model.output_shape == (None, 80, 80, 40, 12)

    model.add(layers.Conv3D(24, (8, 8, 8), strides=(2, 2, 2), padding='same', kernel_initializer=INIT))
    model.add(layers.LeakyReLU(alpha=0.2))
    model.add(layers.BatchNormalization())
    assert model.output_shape == (None, 40, 40, 20, 24)

    model.add(layers.Conv3D(1, (8, 8, 8), strides=(2, 2, 2), padding='same', kernel_initializer=INIT))
    model.add(layers.LeakyReLU(alpha=0.2))
    model.add(layers.BatchNormalization())
...
def train(path, run_name, dataset, epochs, noise_dim, generator, discriminator, generator_optimizer, discriminator_optimizer, seed, checkpoint_manager, cross_entropy, acc_real, acc_fake, num_vox):
  ...
  for epoch in trange(epochs, desc='Epoch'):
    ...
    for image_batch in tqdm(dataset, desc='Iteration'):
      ...
      gen_per_vox_loss = util.train_generator(noise_dim, generator, discriminator, generator_optimizer, cross_entropy, num_vox)
      real_loss, disc_per_vox_loss, fake_loss, real_acc, fake_acc, gradients_of_discriminator = util.train_discriminator(image_batch, noise_dim, generator, discriminator, cross_entropy, acc_real, acc_fake, num_vox)
...
def train_gan(path, run_name, num_vox, max_load):
    
    path_save = path + '_folders'
    os.makedirs(path_save, exist_ok=True)

    train_dataset = util.load_dataset(path, max_load)

    generator = make_generator_model()
    discriminator = make_discriminator_model()

    cross_entropy = tf.keras.losses.BinaryCrossentropy(from_logits=True, label_smoothing=0.02)
    ...
    checkpoint_dir = os.path.join(path_save, 'training_checkpoints', run_name)
    ...
    train(path_save, run_name, train_dataset, EPOCHS, noise_dim, generator, discriminator, generator_optimizer, discriminator_optimizer, seed, manager, cross_entropy, acc_real, acc_fake, num_vox)
