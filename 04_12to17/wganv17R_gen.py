...
def make_generator_model(num_class=2):
    ...
    model.add(layers.Conv3DTranspose(192, (4, 4, 4), strides=(1, 1, 1), padding='same', use_bias=False, kernel_initializer=INIT))
    #model.add(layers.BatchNormalization())
...
def make_discriminator_model(num_class=2):
    ...
    model.add(layers.Conv3D(96, (4, 4, 4), strides=(1, 1, 1), padding='same', kernel_initializer=INIT))
    model.add(layers.LeakyReLU(alpha=0.2))
    #model.add(layers.BatchNormalization())
    assert model.output_shape == (None, 20, 20, 10, 96)
...
def train_gan(path, run_name, num_vox, max_load, full, twohundred):
    ...
    seed_2 = tf.random.normal([100, noise_dim])
    util.save_generated_matrix(path, run_name, generator, 0, seed_2, testID = run_name)
