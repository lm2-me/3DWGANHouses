...
#11R but has 5x5x5 padding with reflect padding instead of zeros
...
def make_generator_model(num_class=2):
    ...
    model.add(layers.Conv3DTranspose(24, (5, 5, 5), strides=(2, 2, 2), padding='same', use_bias=False, kernel_initializer=INIT))
    #model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU())
    assert model.output_shape == (None, 40, 40, 20, 24)
    
    model.add(layers.Conv3DTranspose(12, (5, 5, 5), strides=(2, 2, 2), padding='same', use_bias=False, kernel_initializer=INIT))
    #model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU())
    assert model.output_shape == (None, 80, 80, 40, 12)

    #! confirm this still works, was tanh
    model.add(layers.Conv3DTranspose(num_class, (5, 5, 5), strides=(2, 2, 2), padding='same', use_bias=False, activation='sigmoid'))
...
def make_discriminator_model(num_class=2):
    model = tf.keras.Sequential()
    model.add(layers.Conv3D(12, (5, 5, 5), strides=(2, 2, 2), padding='valid', kernel_initializer=INIT,
                                     input_shape=[160, 160, 80, num_class]))
    model.add(util.ReflectionPadding3D())
    model.add(layers.LeakyReLU(alpha=0.2))
    #model.add(layers.BatchNormalization())
    assert model.output_shape == (None, 80, 80, 40, 12)

    model.add(layers.Conv3D(24, (5, 5, 5), strides=(2, 2, 2), padding='valid', kernel_initializer=INIT))
    model.add(util.ReflectionPadding3D())
    model.add(layers.LeakyReLU(alpha=0.2))
    #model.add(layers.BatchNormalization())
    assert model.output_shape == (None, 40, 40, 20, 24)

    model.add(layers.Conv3D(1, (5, 5, 5), strides=(2, 2, 2), padding='valid', kernel_initializer=INIT))
    model.add(util.ReflectionPadding3D())
    model.add(layers.LeakyReLU(alpha=0.2))
    #model.add(layers.BatchNormalization())
