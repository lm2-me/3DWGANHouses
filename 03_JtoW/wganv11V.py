...
#V uses Leaky ReLU in the generator and critic, RMSProp optimizer, and gradient clipping
...
def train(path, run_name, dataset, epochs, noise_dim, generator, discriminator, generator_optimizer, discriminator_optimizer, seed, checkpoint_manager, acc_real, acc_fake, num_vox):
  ...
  for epoch in trange(epochs, desc='Epoch'):
    ...
    for image_batch in tqdm(dataset, desc='Iteration'):
      ...
      real_loss, disc_per_vox_loss, fake_loss, real_acc, fake_acc, gradients_of_discriminator, gradient_penalty = util.train_discriminator(image_batch, noise_dim, generator, discriminator, acc_real, acc_fake, num_vox, use_grad_pen=False)
      ...
      if check_result: 
        ...
        for w in discriminator.trainable_variables:
           w.assign(tf.clip_by_value(w, -CLIP_CONSTANT, CLIP_CONSTANT))
