...
#wgan v K is a gan using Wasserstein loss, logits, LeakyReLU, and learning rate decay, and using the Root Means Square Prop optimizer
...
def train_gan(path, run_name, num_vox, max_load, full):
    
    train_dataset = util.load_dataset(path, max_load, full)
    ...
    generator_optimizer = tf.keras.optimizers.RMSprop(learning_rate=LR)
